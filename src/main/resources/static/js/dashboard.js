...
fetch(`http://localhost:8080/api/student/dashboard/${user.id}`)
  ...
  .then((data) => {
    ...
    statCards[0].querySelector(".stat-progress-bar.present").style.width =
      data.percentage + "%";
    statCards[1].querySelector(".stat-value").innerText = data.absent;
    statCards[1].querySelector(".stat-progress-bar.absent").style.width =
      100 - data.percentage + "%";
    ...
    const threshold = 75; // or dynamically fetch from settings
    const message =
      data.percentage < threshold
        ? `Warning: Low Attendance (${data.percentage}%)`
        : `Attendance OK (${data.percentage}%)`;
...
function showSection(sectionId) {
  ...
  // If Leave opened → load leave data
  if (sectionId === "leaveReqSection") {
    loadStudentLeaves();
  }
}
...
function changePassword() {
  ...
  fetch("http://localhost:8080/api/student/change-password", {
    method: "PUT",
...
fetch("http://localhost:8080/api/notes/all")
        ...
        data.forEach(note => {
            ...
            const noteCard = `
                <div class="note-card">
                    ...
                    <!-- View PDF in browser -->
                    <a href="http://localhost:8080${note.fileUrl}" 
                       target="_blank" 
                       class="note-btn">
                       👁️ View
                    </a>

                    <!-- Download PDF via download endpoint -->
    ...
    .catch(err => {
        ...
        notesList.innerHTML = `<p style="text-align:center;">Failed to load notes</p>`;
    });
...
function submitLeave() {
  ...
  fetch("http://localhost:8080/api/leave/submit", {
    // ✅ corrected (no 'leaves')
    method: "POST",
    headers: { "Content-Type": "application/json" },
